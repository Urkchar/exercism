...
def hundred(number: int) -> str:
    ...
    # If the number is too great or small, raise an error
    if not (1 <= number <= MAX_INT):
        raise ValueError("Number must be between 1 and 999 (inclusive)")

    # Get the number out of the hundreds place if there is one
    if len(str_number) == 3:
        # Use the first digit to look up the English word, then append " hundred"
        base = HUNDREDS_AND_ONES[str_number[0]] + " hundred"
        # If the number is an even hundred, no need to recur the stuff in the tens and ones place.
        if number % 100 == 0:
            return base
        else:
            # Get the remainder after dividing by 100. This is the stuff in the tens and ones place
            number = number % 100
            return base + " " + hundred(number)

    if len(str_number) == 2:
        # Special case for numbers in the teens
        if 10 <= number <= 19:
            return TEENS[str_number]
        else:
            # Find the index of the tens number in the list
            tens_index = number // 10 - 2   # 20 // 10 - 2 = 0 -> "twenty"
            base = TENS[tens_index]
    ...
    if len(str_number) == 1:
        return HUNDREDS_AND_ONES[str_number]
